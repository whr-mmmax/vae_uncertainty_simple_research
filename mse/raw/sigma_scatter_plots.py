...
def compute_stats(x: np.ndarray, y: np.ndarray):
    ...
    if np.ptp(x) == 0 or np.ptp(y) == 0:
        _nan_result['degenerate_reason'] = (
            'zero-variance x' if np.ptp(x) == 0 else
            'zero-variance y'
        )
        return _nan_result
...
def render_partial_comparison_figure(all_decompose_stats: dict, output_dir: str):
    ...
    if all_A:
        ax.set_ylim(min(all_A) - 0.12, max(all_A) + 0.14)
    ...
    for (bar, fv), ci in zip(zip(bars_f, fractions), frac_ci):
        if np.isfinite(fv):
            ci_hi = ci[1] if (ci and np.isfinite(ci[1])) else fv
            y_anchor = max(fv, ci_hi) + (1.2 if fv >= 0 else -1.2)
    ...
    _pstars(ax2, x, fractions, frac_ci, frac_ps, fontsize=8.0, gap_frac=0.04)
    ...
    frac_hi   = [ci[1] for ci in frac_ci if np.isfinite(ci[1])]
    if finite_f:
        f_top = max(max(finite_f), max(frac_hi) if frac_hi else 0.0)
        ax2.set_ylim(min(0.0, min(finite_f)) - 5.0, f_top * 1.2)
    ...
    ], fontsize=8, framealpha=0.92, loc='upper left',
