...
def fisher_z_ci(rho_list: np.ndarray, alpha: float = 0.05):
    """用 Fisher z 变换计算 Spearman ρ 均值的置信区间"""
...
def _to_serial(obj):
    """将 numpy 类型递归转换为 Python 原生类型，便于 JSON 序列化"""
...
def build_latent_cache(img_paths: list) -> list:
    """
    对每张图像做一次 VAE 前向，缓存原始 σ 和重建误差。
    返回列表，每个元素为 (sigma, rec_err) 或 None。
    """
    vae   = get_vae()
    cache = []
    for path in tqdm(img_paths, desc="Building latent cache"):
        try:
            ...
            with torch.no_grad():
                ...
                log_sigma = 0.5 * posterior.logvar       # (1,4,H_l,W_l)
                sigma     = torch.exp(log_sigma)         # (1,4,H_l,W_l) 原始 σ

                mean_f = posterior.mean
                z = mean_f + sigma * torch.randn_like(mean_f)
                ...
                sigma_bad = (torch.isnan(sigma).any() or torch.isinf(sigma).any())
                rec_bad   = (torch.isnan(rec_err).any() or torch.isinf(rec_err).any())
                has_bad   = sigma_bad or rec_bad

            if has_bad:
                reason = ("sigma " if sigma_bad else "") + ("rec_err" if rec_bad else "")
                print(f"  ⚠  {Path(path).name}: NaN/Inf in [{reason.strip()}] — skipped")
                cache.append(None)
                continue

            cache.append((
                sigma.squeeze(0).cpu().numpy(),    # (4, H_l, W_l)
...
def compute_patch_stats_mean(sigma_np: np.ndarray, re_np: np.ndarray,
                              pixel_patch_size: int):
    """先将4个通道的σ取平均，再提取patch统计量（均值）"""
    lps = max(1, pixel_patch_size // 8)
    s  = torch.from_numpy(sigma_np).unsqueeze(0)         # (1,4,H_l,W_l)
    re  = torch.from_numpy(re_np).unsqueeze(0)           # (1,1,H_l,W_l)
    s_mean = s.mean(dim=1, keepdim=True)                 # (1,1,H_l,W_l)
    sp  = F.unfold(s_mean, kernel_size=lps, stride=lps)
    rp  = F.unfold(re,      kernel_size=lps, stride=lps)
    return sp.mean(dim=1).squeeze().numpy(), rp.mean(dim=1).squeeze().numpy()


def compute_patch_stats_per_channel(sigma_np: np.ndarray, re_np: np.ndarray,
                                     pixel_patch_size: int):
    ...
    for c in range(4):
        sc = s[:, c:c+1, :, :]
        sp = F.unfold(sc, kernel_size=lps, stride=lps)
...
def make_parser(description: str = "VAE σ correlation analysis") -> argparse.ArgumentParser:
    ...
    p.add_argument('--log',        type=_str2bool, default=None,
                   help='Whether σ is log-transformed; written to JSON. '
                        ...
                        'Default: Config.LOG = False (raw σ in this module)')
    return p